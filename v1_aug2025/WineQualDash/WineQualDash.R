...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    p("Enter actual measurement values below.", 
      style = "padding: 0 15px; font-size: 12px; color: #888;"),
    p("NOTE: Values are normalized among the training data, and the new attribute 'free.SO2.ratio' is automatically calculated, before input into the predictive model.", 
      style = "padding: 0 15px; font-size: 10px; color: #666; font-style: italic;"),
...
server <- function(input, output, session) {
  ...
  model <- reactive({
    if (file.exists("fwf.Rds")) {
      readRDS("fwf.Rds")
    } else {
      showNotification("Error: fwf.Rds model file not found!", 
  ...
  trainData <- reactive({
    ...
    if (file.exists("trainClean3.csv")) {
      read.csv("trainClean3.csv", stringsAsFactors = FALSE) %>%
        mutate(
          type = factor(type),
          location = factor(location),
          quality = factor(quality, ordered = TRUE)
        )
    } else if (file.exists("trainClean.csv")) {
      ...
      showNotification("Warning: Training data file not found. Using mock data for normalization.", 
  ...
  observeEvent(input$predict, {
    ...
    if (is.null(model())) {
      showNotification("Model not loaded. Please check that fwf.Rds exists.", 
    ...
    prediction$normalized_values <- new_obs_normalized[, num_cols[num_cols %in% names(new_obs_normalized)]]
    
    # Use the actual trained model for predictions
    tryCatch({
      # Get predictions from the polr model
      ...
      # If pred_probs is a matrix (multiple predictions), take the first row
      if (is.matrix(pred_probs)) {
        probs_vector <- pred_probs[1, ]
      } else {
        # If it's already a vector, use it directly
        probs_vector <- pred_probs
      }
      
      # Ensure we have probabilities for quality levels 3-9
      # The model might return probabilities with names like "3", "4", etc.
      quality_levels <- as.character(3:9)
      
      # Create a vector with all quality levels, filling in zeros if needed
      final_probs <- numeric(length(quality_levels))
      names(final_probs) <- quality_levels
      
      # Fill in the probabilities from the model
      for (i in seq_along(quality_levels)) {
        level <- quality_levels[i]
        if (level %in% names(probs_vector)) {
          final_probs[i] <- probs_vector[level]
        }
      }
      
      # Store predictions
      prediction$quality <- as.character(pred_class)
      prediction$probs <- final_probs
      
      # Show notification
      showNotification("Quality prediction completed using trained model!", 
                       type = "message", 
                       duration = 3)
      
    }, error = function(e) {
      showNotification(paste("Error in prediction:", e$message), 
                       type = "error", 
                       duration = 5)
  ...
  output$model_info <- renderPrint({
    if (!is.null(prediction$quality)) {
      ...
      if (!is.null(model())) {
        cat("Model loaded: Ordinal Logistic Regression (polr)\n")
        cat("Model file: fwf.Rds\n")
      } else {
        cat("Model not loaded. Please check fwf.Rds file.\n")
      }
    }
  })
  ...
  output$normalization_info <- renderPrint({
    if (!is.null(prediction$normalized_values)) {
      ...
      cat("and applying scale() to maintain consistency with model training.")
    } else {
      ...
      cat("3. All numeric variables (including free.SO2.ratio) are normalized using scale()\n")
      cat("4. The normalized values are fed to the polr model for prediction")
    }
  })
}
