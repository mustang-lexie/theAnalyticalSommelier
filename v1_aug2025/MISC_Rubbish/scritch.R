...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    # Radio Buttons for Categorical Attributes
...
server <- function(input, output, session) {
  ...
  # Load the training data for normalization step
  trainData <- reactive({
  ...
  observeEvent(input$predict, {
    ...
    # Get the training data from file retrieval
    ...
    # Load and use your actual trained model
    model <- readRDS("fwf.Rds")
    ...
    # Store predictions - convert matrix to vector for consistency with original code
    prediction$quality <- as.character(pred_class)
    # Extract as vector and keep the names as attributes
    prediction$probs <- as.numeric(pred_probs)
    prediction$quality_levels <- colnames(pred_probs)
    
    # Show notification
    showNotification("Quality predicted!", 
                     type = "message", 
                     duration = 3)
  })
  ...
  output$prob_plot <- renderPlot({
    if (!is.null(prediction$probs)) {
      ...
      print(paste("Class of prediction$probs:", class(prediction$probs)))
      print(paste("Length of prediction$probs:", length(prediction$probs)))
      print(paste("Probs values:", paste(round(prediction$probs, 3), collapse=", ")))
      
      # Use the quality levels stored separately
      if (!is.null(prediction$quality_levels)) {
        quality_levels <- prediction$quality_levels
      } else {
        # Fallback - create levels based on length of probs
        quality_levels <- as.character(seq(3, 3 + length(prediction$probs) - 1))
      }
      
      print(paste("Quality levels:", paste(quality_levels, collapse=", ")))
      
      # Ensure we have the right number of labels
      if (length(quality_levels) != length(prediction$probs)) {
        print(paste("ERROR: Mismatch! Levels:", length(quality_levels), "Probs:", length(prediction$probs)))
        quality_levels <- as.character(seq_along(prediction$probs))
      }
      
      # Create gradient colors for bars
      colors <- ifelse(quality_levels == prediction$quality, 
                       "darkred", "goldenrod")
      
      # Simple, clean barplot
      bp <- barplot(prediction$probs,
      ...
      # Add percentage labels using the positions returned by barplot
      text(x = bp,
